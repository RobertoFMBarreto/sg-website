...
const ZigZagBall = () => {
  ...
  useEffect(() => {
    ...
    // Animação para o movimento da bola em zigue-zague
    gsap.to(ballRef.current, {
      keyframes: { x: [0, 200, -200, 200, -200, 0] }, // Movimento zigue-zague
      y: "100vh", // A bola vai descer ao longo da altura da página
      scrollTrigger: {
        trigger: "body", // Não limitamos a uma seção específica
        start: "top top", // Começa quando a página rola desde o topo
        end: "bottom bottom", // Termina quando chega ao fim da página
        scrub: 1, // Sincroniza o movimento com o scroll
        markers: false, // Se quiseres ver onde começa e termina, podes ativar (marcadores)
      },
    });

    return () => {
      // Limpeza ao final da animação
      ScrollTrigger.getAll().forEach((trigger) => trigger.kill());
    };
  }, []);

  return (
    <div className="flex-1 relative animate-slide-up z-10">
      {/* Rotating CPU with Enhanced Electronic Circuit Animation */}
      <div
        ref={cpuRef}
        className="relative w-full aspect-square max-w-md mx-auto transition-transform duration-300"
      ...
      >
        ...
        <div className="absolute inset-0 z-10">
          ...
          <div className="absolute top-[35%] right-0 w-3 h-3 bg-yellow-400 rounded-full shadow-[0_0_15px_5px_rgba(250,204,21,0.6)] animate-pulse-subtle"></div>
      ...
      <div className="absolute inset-0 pointer-events-none">
        {[...Array(8)].map((_, i) => {
          const angle = i * 45;
          const isActive = true; //Math.floor((rotation % 360) / 45) === i;
          return (
            <div
              key={`marker-${i}`}
